...
public class BankWithdraw {
	...
	public static void run(CommandSender sender, String[] args) {
		...
		guild.withdraw(d);
		GuildMain.getInstance().getEcon().depositPlayer(Bukkit.getOfflinePlayer(user.getUniqueId()), d);
		for (User u : GuildMain.getInstance().getUsers()) {
			if (u.getGuild().equalsIgnoreCase(guild.getName())) {
				u.getPlayer().sendMessage(guild.getPrefix() + ChatColor.GREEN + sender.getName() + ChatColor.GOLD + " has withdrew $" + ChatColor.GREEN + d + ChatColor.GOLD + " into the guild bank.");
			}
		}
	}
